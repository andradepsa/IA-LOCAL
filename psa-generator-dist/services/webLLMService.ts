...
// Usa /hf-proxy se disponível (resolve CORS do HuggingFace redirect)
export async function loadModel(modelId: string = DEFAULT_MODEL): Promise<boolean> {
    ...
    try {
        ...
        // Se estiver no Cloudflare, configura para usar o proxy HF
        if (useProxy) {
            console.log('[WebLLM] Usando proxy /hf-proxy para HuggingFace (CORS workaround)');
            // Cria engine com configuração customizada de modelo
            // que redireciona todas as requisições HF para nosso proxy
            const originalModel = modelId;
            const proxyModelId = modelId;

            // Monkey-patch no fetch para redirecionar HF → /hf-proxy
            const originalFetch = window.fetch;
            window.fetch = async function(input: any, init?: any) {
                const url = typeof input === 'string' ? input : input?.url;
                if (url && url.includes('huggingface.co')) {
                    // Substitui https://huggingface.co/X por /hf-proxy/X
                    const hfPath = url.replace('https://huggingface.co/', '');
                    const proxyUrl = `/hf-proxy/${hfPath}`;
                    console.log(`[WebLLM] Proxy: ${url.slice(-50)} → ${proxyUrl}`);
            ...
            try {
                engine = await webllm.CreateMLCEngine(proxyModelId, modelConfig);
            } finally {
                // Restaura fetch original
                window.fetch = originalFetch;
            }
        } else {
            // Modo direto (sem proxy) — para desenvolvimento local
            engine = await webllm.CreateMLCEngine(modelId, modelConfig);
        }
